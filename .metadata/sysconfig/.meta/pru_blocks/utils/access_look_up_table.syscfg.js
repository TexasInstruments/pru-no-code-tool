...
function validate(inst, report) {
    ...
    if (inst["input1"].length == 0) {
        ...
        if (inputInst && inputInst["constant1"] !== undefined) {
            ...
            // Get the referenced LUT instance to check table size
            const lutModule = system.modules["/pru_blocks/utils/look_up_table"];
            if (lutModule && lutModule.$instances) {
                const referencedLUT = lutModule.$instances.find(lut => lut.$name === inst["lutReference"]);
                if (referencedLUT) {
                    ...
                    else if (inst["lutReference"]) {
                        report.logError(`Referenced lookup table '${inst["lutReference"]}' not found`, inst);
                    } 
                }
            }
        }
...
function getLongDescription() {
    ...
    ### Configuration Steps
    1. Select which Lookup Table block to read from (dropdown below)
    2. Connect an index value to the "index" input port
    3. The output data size automatically matches the selected table's data type (byte/ushort/uint)
...
exports = {
    ...
    config: [
        ...
        },
        {
            ...
            hidden: true,
            getValue: (inst) => {
                // Get byte size from the referenced LUT's dataType
                const lutModule = system.modules["/pru_blocks/utils/look_up_table"];
                if (lutModule && lutModule.$instances && inst.lutReference) {
                    const referencedLUT = lutModule.$instances.find(lut => lut.$name === inst.lutReference);
                    if (referencedLUT) {
                        if (referencedLUT["dataType"] == "byte") return "1";
                        if (referencedLUT["dataType"] == "ushort") return "2";
                        if (referencedLUT["dataType"] == "uint") return "4";
                    }
                }
        ...
        {
            name: "lutReference",
            displayName: "Select Lookup Table",
            description: "Choose which lookup table to access",
            default: "",
            options: (inst) => {
        ...
        },
        {
            ...
            hidden: true,
            getValue: (inst) => {
                // Get dataType from the referenced LUT
                const lutModule = system.modules["/pru_blocks/utils/look_up_table"];
                if (lutModule && lutModule.$instances && inst.lutReference) {
                    const referencedLUT = lutModule.$instances.find(lut => lut.$name === inst.lutReference);
                    if (referencedLUT) {
                        if (referencedLUT["dataType"] == "byte") {
                            return "1";
                        }
                        if (referencedLUT["dataType"] == "ushort") {
                            return "2";
                        }
                        if (referencedLUT["dataType"] == "uint") {
                            return "4";
                        }
                    }
                }
                return "0"; // Default to 1 byte
            }
        },
        {
            name: "noOfCycles",
